...
x[x < 0.9] = 0
x1= np.random.random((128, 128, 128))
x1[x1 < 0.9] = 0
...
sparse=torch.sparse.FloatTensor(x_i.t(), x_v, torch.Size([size_x_i[0] + 1,128,128,128]))
...
def convert_list_of_tensors_to_sparse_array(data_in):
    ...
    sparse_matrix = torch.sparse.FloatTensor(index.t(), data, torch.Size([index_size[0] + 1, 128, 128, 128]))
